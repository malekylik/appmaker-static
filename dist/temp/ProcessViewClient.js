...
const loadProcessID = promosifyClientCall((...args) => app.datasources.ProcessInfoByID.load(...args));
const loadRiskAssessmentID = promosifyClientCall((...args) => app.datasources.RiskAssessmentByID.load(...args));
const loadProcessesWithRA = promosifyClientCall((...args) => app.datasources.FilteredProcessWithRA.load(...args));
const loadRiskDriverByProcessId = promosifyClientCall((...args) => app.datasources.RiskDriverByProcessId.load(...args));
const loadControlsByRiskID = promosifyClientCall((...args) => app.datasources.ControlsByRiskID.load(...args));
const loadGapsByControlID = promosifyClientCall((...args) => app.datasources.GapsByControlID.load(...args));
const surveyByProcessID = promosifyClientCall((...args) => app.datasources.PreRASurveyResultsByProcessID.load(...args));
const questionnaireAnswersByProcessID = promosifyClientCall((...args) => app.datasources.RAQuestionnaireAnswersByProcessID.load(...args));
const questionnaireResultsByProcessID = promosifyClientCall((...args) => app.datasources.RAQuestionnaireResultsByProcessID.load(...args));
...
function isQuestionnaireInProgress() {
  return app.datasources.RAQuestionnaireResultsByProcessID.items.size !== 0 && app.datasources.RAQuestionnaireResultsByProcessID.item.status === ModelStatus.InProgress;
}

function isQuestionnaireCompleted() {
  return app.datasources.RAQuestionnaireResultsByProcessID.items.size !== 0 && app.datasources.RAQuestionnaireResultsByProcessID.item.status === ModelStatus.Done;
}
...
function updateQuestionnaireUI() {
  const tableBody = app.view.getDescendant('QuestionnaireTableBody');
  const rows = tableBody.getDescendantsByClass('app-quesionnaireTableRow');
  const answers = app.datasources.RAQuestionnaireAnswersByProcessID.items.toArray();
  ...
  for (let i = 0; i < answers.length; i++) {
    ...
    let index = map[answer.QuestionNumber];
...
function isProcessApprovalButtonVisible(processLoading, processLoaded, gpo) {
  // TODO: check maybe it won't update if FilteredProcessWithRA is changed
  // TODO: should be the latest ra (what does it mean "the latest ra")
  const processID = getProcessIDofPage();
  const ra = app.datasources.FilteredProcessWithRA.items ? app.datasources.FilteredProcessWithRA.items.toArray().filter(ra => ra.ProcessID === processID)[0] : undefined;
...
function onProcessApproveHandler(widget) {
  ...
  const processID = getProcessIDofPage();
  const raids = app.datasources.FilteredProcessWithRA.items ? app.datasources.FilteredProcessWithRA.items.toArray().filter(ra => ra.ProcessID === processID).map(ra => ra.RAID) : [];
...
function loadPageData() {
  if (!app.datasources.FilteredProcessWithRA.loading && !app.datasources.FilteredProcessWithRA.loaded && getProcessIDofPage()) {
    loadProcessesWithRA();
  }
